    ...
    ui(new Ui::MainWindow)
{
    ...
    this->setWindowTitle("Ведомость приемки и отгрузки товаров");
    QRect rect = frameGeometry();
    rect.moveCenter(QDesktopWidget().availableGeometry().center());
    move(rect.topLeft());
    this->setGeometry(rect);

// Переопределение сигнала окончания ввода
// связывание с функцией поиска
...
void MainWindow::initWindow(QTreeWidget *treeWidget, QLineEdit *lineEdit, QFont &f, QValidator *vld){
	...
	// установка подсказки для формата ввода данных
		lineEdit->setPlaceholderText("304DB75F196000188001C13A");
